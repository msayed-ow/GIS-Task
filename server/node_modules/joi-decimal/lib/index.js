...
// TODO: change precision behavior
// TODO: add valid = equal
// TODO: add invalid = not = disallow
module.exports = joi => ({
  ...
  language: {
    ...
    only: '!!"{{value}}" didn\'t match any of the allowed values "{{values}}"',
  ...
  pre(value, state, prefs) {
    ...
    if (prefs.convert) {
      if (this._flags.sd) {
        decValue = new Decimal(decValue.toPrecision(
          this._flags.sd,
  ...
  rules: [
    ...
    },
    {
      ...
      validate(params, value, state, prefs) {
        ...
        console.log(Decimal.isDecimal(value));
        if (!value.gt(limit)) {
    ...
    /*
    {
      name: 'valid',
      description(params) {
        return `Value should be one of ${params.values}`;
      },
      params: {
        values: joi.any().items(
          joi.alternatives([
            joi.object().type(Decimal),
            joi.number().allow(Infinity, -Infinity, NaN, -NaN).strict(),
            joi.string(),
      ...
      validate(params, value, state, prefs) {
        ...
        if (joi.isRef(values)) {
          // resolve
          values = values(state.reference || state.parent, prefs);
          // validate
          const noRefSchema = joi.alternatives([
            joi.object().type(Decimal),
            joi.number().allow(Infinity, -Infinity, NaN, -NaN).strict(),
          ...
          const result = noRefSchema.validate(values);
          if (result.error) {
            return this.createError('decimal.ref', { value, values }, state, prefs);
          }
        }
        console.log(values);
        ...
        for (let val in values) {
          val = new Decimal(val);
          if (!value.eq(val)) {
            return this.createError('decimal.only', { value, values }, state, prefs);
          }
        }
